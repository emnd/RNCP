(function() {
	...
	app.directive("actor", function(){
		return {
			restrict: 'E',
			templateUrl: "searchByActor.html",
			controller: 
				function($http) {
				var self = this;
				self.title = "search actor Ctrl";
...
//				self.query = "&query="+self.searchActors;
//				self.PAGE = "&page=1";
//				self.INCLUDE = "&include_adult=false";
				...
				// recherche par acteurs
					 ...
					 self.getActors = function() {
							...
							console.log("J'ai cliqué sur rechercher par acteur ");					
							console.log(self.searchActors);
							
							$http({
								method : 'GET',
								url : 'http://localhost:8080/theMovieDbApi/services/getFilmByActorName/'+self.searchActors
